...
alter table public.season_question_seen
  alter column served_to set not null;

alter table public.season_question_seen
  drop constraint if exists season_question_seen_pkey;

alter table public.season_question_seen
  add constraint season_question_seen_pkey primary key (season_id, question_id, served_to);

create index if not exists season_question_seen_player_recent_idx
  on public.season_question_seen(season_id, served_to, served_at desc);
...
create table if not exists public.bot_action_log (
  ...
  primary key (season_id, bot_id, acted_on)
);
...
begin
  ...
  if p_action_type = 'home' then
    ...
    values (p_season_id, p_territory_id, v_user)
    on conflict do nothing;
...
-- game_submit_answer, run_test_bot_turns, and run_daily_tick are applied in the live migration
-- and are intentionally kept in the repository migration generated for this release.
