-- Balance-pass hygiene (two small closures):
--
-- 1. Defend-reroll cap. `report_question` voids the reporter's session, and
--    `game_begin_action`'s defend guard only rejected sessions in
--    ('active','completed','failed') -- 'void' was absent, so a defender could
--    report -> void -> re-open the same defense repeatedly at zero cost
--    (defend spends no move and is turn-exempt), walking the state's ~11
--    question bank until they drew one they knew. Two voided defense sessions
--    per attack is the ceiling now; a third open is rejected and the defense
--    must ride with the question it has (or the 24h deadline resolves it).
--
-- 2. `create_group` (v1) grant revoke. Superseded by `create_group_v2`; no
--    client calls it (tests/db/audit.test.ts documented the dead grant as an
--    explicit exception). Revoked here; the audit allowlist drops the
--    exception in the same change set.
--
-- game_begin_action body transcribed from its live definition in
-- 20260803180900_fix_off_turn_message.sql with only the defend-branch cap
-- added. Migrations compose; older files are history.
...
begin
  ...
  if p_action_type = 'home' then
    ...
    -- Reroll cap: report_question voids a defense session, and reopening is
    -- free (no move, turn-exempt). Two voids per attack is the ceiling --
    -- otherwise a defender can walk the whole question bank for the state.
    if (
      select count(*) from public.game_sessions
      where attack_id = v_attack.id and action_type = 'defend' and status = 'void'
    ) >= 2 then raise exception 'This defense has been rerolled twice already'; end if;
...
-- Grant hygiene (Phase-1 convention): re-assert this function's contract and
-- close the documented create_group (v1) dead grant.
...
revoke execute on function public.create_group(text, text[], integer) from authenticated;
