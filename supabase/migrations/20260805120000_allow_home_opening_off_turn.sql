-- Audit F4 (2026-08-05): a mid-season joiner's opening move was blocked while
-- off-turn. The dock advertised "OPENING MOVE - Play question" but both the
-- turn gate in game_begin_action and the enforce_test_turn_session trigger
-- rejected 'home' sessions for off-turn players. Securing home ground touches
-- only the player's own state, consumes no action and mirrors defend's
-- exemption, so 'home' joins 'defend' outside the turn gate. The client-side
-- waiting overlay gets the matching exemption in the same commit.

CREATE OR REPLACE FUNCTION public.game_begin_action(p_season_id uuid, p_territory_id text, p_action_type text, p_attack_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  -- action except defend and home, which are exempt from the turn gate.
  if v_test and p_action_type not in ('defend', 'home') and v_season.current_turn_user_id is distinct from v_user then
...
begin
  ...
  if coalesce(v_test, false)
     and new.action_type not in ('defend', 'home')
