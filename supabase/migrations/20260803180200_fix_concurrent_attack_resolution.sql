-- Finding 5: two players could each open an attack session on the same state,
-- because the contested check ran only at begin time and nothing reserved the
-- row. The first streak to finish inserted the `attacks` row; the second
-- violated `one_contested_attack_per_territory` and the raw Postgres error
-- reached the player, rolling back their winning answer while the move they
-- spent at begin stayed spent.
--
-- The winning answer now re-reads the territory under a lock before it inserts
-- the attack. A player who loses the race gets a clean `void` outcome and their
-- move back, the same treatment `report_question` already gives.
