...
| 6 | bug | `run_test_bot_turns` (20260802173100) | `SECURITY DEFINER` with no caller check at all — it validates only that the season is active and the group is `test_mode`. Its `revoke all on function ... from public` does not remove the `anon`/`authenticated` grants Supabase's default privileges hand to every new function, so the function is reachable **unauthenticated**. **Fixed:** the function is an internal helper -- `run_daily_tick` drives it and no client calls it -- so EXECUTE is revoked from `public, anon, authenticated` and granted only to `service_role`. An `auth.uid()` caller check was deliberately *not* added: the tick runs with no authenticated user and would have broken. | `tests/db/audit.test.ts` "run_test_bot_turns is not reachable by any client role". Before the fix: `has_function_privilege('anon', ...)` → `t`; anon client `rpc('run_test_bot_turns')` → `{"actions":0}` with no error; a plain (non-commissioner) member call also succeeded. | fixed (a8e1d49) |
| 7 | bug | `end_test_turn` (20260730220000) | Same ineffective revoke pattern (`revoke all ... from public` only). `anon` retains EXECUTE and reaches the function body; it is saved from abuse only by the internal `if v_user is null then raise 'Authentication required'`. The grant, not the guard, is the thing that is wrong — the same mistake produced finding 6, which has no guard. | `has_function_privilege('anon', 'public.end_test_turn(uuid)', 'EXECUTE')` → `t`; anon `rpc('end_test_turn')` returns "Authentication required" rather than a permission error. Compare `run_daily_tick`, which used `revoke ... from public, anon, authenticated` and correctly returns "permission denied for function run_daily_tick". **Fixed at the pattern, not just the two functions:** `end_test_turn`, `run_test_bot_turns`, `enforce_test_turn_session` and `sync_question_attempt_stats` all had the ineffective revoke; all four are now revoked from `anon` (and from `authenticated` where no client needs them). `public.security_definer_grants()` was added so `tests/db/audit.test.ts` "no security-definer function in public is executable by anon" enumerates every security-definer function on every run and fails the moment a future migration re-arms the trap. | fixed (a8e1d49) |
| 8 | bug | `game_begin_action` 'fortify' branch | The `fortify_log` row is written at *begin* time, so a fortify lost to a wrong answer still consumes the day's one fortify for that state — and the action too. The player is told "You already fortified this state today" when they in fact failed it. **Fixed:** `game_begin_action` only checks the log now; the winning answer claims the day. That also closes the opposite gap — the insert, not the check, decides, so two fortify sessions on one state cannot both succeed and the loser gets its move back. | `tests/db/audit.test.ts` "a fortify lost to a wrong answer can be retried the same day". Before the fix: begin a fortify, answer wrong (`status: failed`), begin the same fortify again → "You already fortified this state today". | fixed (3ff489c) |
| 9 | bug | `report_question` (initial schema) | A single report sets `questions.active = false` **globally**, with no threshold, no reviewer, and no per-season scoping. The bank holds only 11 questions per state, shared by every league, so a handful of reports permanently strips a state's trivia from the whole product. `question_reports` has no uniqueness constraint, so one account can report repeatedly. **Fixed:** three distinct reporters are now required before `questions.active` flips, a unique index on `(question_id, reported_by)` makes one account count once, and the migration re-activates any question an under-threshold report already knocked out. The reporter still gets the session voided and the move refunded on their first report (fortify included, since it now costs a move), and `game-runtime-controls.tsx` reports the RPC's own message instead of always claiming a quarantine. | `tests/db/audit.test.ts` "report_question voids and refunds immediately but quarantines only on the third distinct reporter". Deactivating all of a state's questions makes `game_begin_action` fail with "This state has no active questions" (cleanly — the move is refunded by the rollback). | fixed (e1f1f90) |
| 10 | bug | `refresh_player_actions` (20260802173000) | The function opens with `perform public.run_daily_tick()`, and `run_daily_tick` loops **every active season in the database**. `refresh_player_actions` is called by `group_snapshot` and by `game_begin_action`, so every page load and every move runs a full cross-tenant scoring pass. The UI amplifies it: `GameRuntimeControls` polls `group_snapshot` every 5s and `TerritoryGameV2` every 20s, per open tab. It also creates a cross-group deadlock surface (each caller updates other groups' `player_actions` rows while holding locks on its own). | Timed on the local stack with 50 active seasons: `run_daily_tick()` = 7.2ms idle, 13.5ms when scoring is due (108 players). Cost is O(all active seasons) per snapshot, so it grew with total product usage rather than with the caller's own league. **Fixed:** the `perform run_daily_tick()` is gone; `/api/cron/tick` is the only day-advancer, which is what `vercel.json` always intended. Sequenced after finding 2 so the one remaining advancer is the secure one. | `tests/db/audit.test.ts` "a page load does not run the daily tick for every other league in the database". | fixed (c8cc9bc) |
| 11 | bug | `seasons.last_scored_on`, `player_actions.last_refresh_on`, `fortify_log.played_on` | Mixed day boundaries. All three columns DEFAULT to `current_date`, which is the database's **UTC** date, while `run_daily_tick` and `refresh_player_actions` compare against `(now() at time zone groups.timezone)::date`. A season started while the UTC date is already ahead of the group's local date (17:00–24:00 PT for the default `America/Los_Angeles`) stamps a `last_scored_on` one day in its own future and silently skips a scoring day. `test_refill_actions` and `end_test_turn` also clear `fortify_log` by UTC `current_date`, so the once-per-day fortify rolls over at UTC midnight while actions roll over at group-local midnight. DST itself is handled correctly — the zone is named, not an offset — the defect is the unit mismatch. | `tests/db/audit.test.ts` "every day-boundary column is stamped in the group's local day, not UTC". Column defaults confirmed via `information_schema.columns`: all three are `CURRENT_DATE`. **Fixed:** every writer (`start_season`, `end_test_turn`, `test_refill_actions`, `game_begin_action`, `game_submit_answer`) now stamps `public.group_local_date(group_id)`; the defaults stay only as a fallback nothing relies on. The probe is deterministic by construction — it runs the whole flow in a fixed UTC-12 zone and a fixed UTC+14 zone, which are always on different calendar days, so it discriminates at any time of day. | fixed (9ab5883) |
| 12 | papercut | `groups.timezone` | The column exists, defaults to `'America/Los_Angeles'`, and drives every day boundary in `run_daily_tick` / `refresh_player_actions` / `run_test_bot_turns` — but nothing ever sets it. `create_group_v2` takes no timezone parameter and no UI or RPC writes it, so every league in the product scores on Pacific time regardless of where its players live. **Backlogged (P2-timezone):** the fix is a new `create_group_v2` parameter plus a create-league form field plus a per-group cron story (`vercel.json` runs one `5 8 * * *` UTC schedule) — past the ~10-line papercut budget. Finding 11 added `public.group_local_date()`, which reads the column correctly, so only the writer is missing. | `grep -rn timezone app components lib supabase/migrations` returns only the column definition and the read sites. | backlogged (6bb4830) |
| 13 | bug | `app/api/**` | Seven of the eight API routes are dead code. The UI calls Supabase RPCs directly (`components/territory-game-v2.tsx`, `components/game-runtime-controls.tsx`); `grep -rn "/api/" app components lib` outside `app/api/` returns nothing. Only `/api/cron/tick` is wired, via `vercel.json`. They have also drifted: `app/api/groups/route.ts` calls the superseded `create_group` v1 (no `test_mode`, no `opening_mode`, and a 14/30/60 season-length whitelist the v2 RPC no longer uses), so anything that did call it would create a league the current UI cannot start. Auth itself is correct in all seven — each uses `lib/supabase/server.ts` and `getUser()` before the RPC. **Backlogged (P1-task9):** deleting the routes is Task 9's cleanup scope, and the README corrections in finding 18 should land with the same change. | `grep -rn "/api/" app components lib \| grep -v '^app/api/'` → no matches. `app/api/groups/route.ts:20` calls `create_group`; the UI calls `create_group_v2` at `components/league-entry.tsx`. Re-verified in task 9 before deleting all seven. | fixed (8bf8aae) |
| 14 | bug | `supabase/functions/test-signup/index.ts` | Account-takeover path. When the submitted email matches an **existing unconfirmed** account, the function calls `admin.auth.admin.updateUserById` to set the caller's password and `email_confirm: true` — so anyone holding a valid playtest invite code can seize any unconfirmed account by knowing its address. The invite-code gate itself is correct (the group is looked up and `test_mode` verified *before* any account is created), the service key is never returned, and errors do not leak it. Also worth tightening: `Access-Control-Allow-Origin: *` on an endpoint that creates confirmed accounts. **Fixed:** signup no longer touches an account that already exists. An invite code proves the holder may join *that league*; it proves nothing about who owns an email address. `createUser` now runs unconditionally and a duplicate email is mapped to a 409 "Sign in instead". The wildcard CORS origin is backlogged (P2-signup-cors), not fixed here. | `tests/db/signup.test.ts` "signup cannot take over an existing unconfirmed account", which drives the deployed function over HTTP against the local stack. Before the fix it returned `200 {"ok":true,"recovered":true}` and confirmed the victim's account with the attacker's password. | fixed (9e64625) |
| 15 | papercut | `supabase/functions/test-signup/index.ts` | The existing-account lookup is `listUsers({ page: 1, perPage: 1000 })` and scans only that first page. Past 1000 users a returning player is treated as new and `createUser` fails with a duplicate-email error instead of the intended recovery path. Use `getUserByEmail`/a filtered query instead of paging the whole user table. **Fixed as a side effect of finding 14:** the `listUsers` scan is gone entirely — `createUser` rejects the duplicate itself, which is correct at any user count. | Code review of `index.ts:48-51`. | fixed (9e64625) |
| 16 | bug | tooling (`npm run lint`) | `eslint .` crashes repo-wide on the first file, so the lint gate provides zero coverage: `TypeError: Error while loading rule 'react/display-name': contextOrFilename.getFilename is not a function` (`eslint-plugin-react/lib/util/version.js:31`). ESLint 10.8.0 + `eslint-config-next` 16.2.9. Pre-existing, not introduced by this phase — but Task 11 runs lint as a closeout gate, so it must be resolved (pin/upgrade the plugin, or drop `react/display-name`) rather than deferred. **Fixed:** the crash is `eslint-plugin-react`'s default `version: "detect"`, which calls the `context.getFilename()` ESLint 10 removed. `eslint.config.mjs` now names the version (read from the installed `react/package.json`, so it cannot drift), which skips detection entirely and repairs every rule the plugin contributes rather than just `react/display-name` — and needs no new dependency. | `npm run lint` → crash on `app/api/cron/tick/route.ts` before the fix; reproduced on any single file, including untouched ones. Afterwards the gate runs and reports 3 real `react-hooks/set-state-in-effect` errors plus 1 `exhaustive-deps` warning, all inside `components/territory-game-v2.tsx` — backlogged as P1-task9, since fixing them means restructuring the exact effects Task 9 splits up. | fixed (b8633ed) |
...
| 18 | papercut | `README.md` | Five stale claims that would mislead the next contributor. (a) "Three-player minimum" — `start_season` requires two humans. (b) "No coast-to-coast or sport-diversity bonus yet" — `run_daily_tick` awards both (3 and 2 points). (c) "Daily scoring is UTC rather than group-local time" — scoring now uses `groups.timezone`; only the column defaults are still UTC (finding 11). (d) "Server-side answer checking; correct answers are not sent to the browser" — false, see finding 1; `game_submit_answer` also returns `correct_answer` on every response, including mid-streak. (e) "Installable PWA shell" — `components/pwa-register.tsx` is imported nowhere and `app/layout.tsx` emits no `<link rel="manifest">`, so `public/sw.js` and `public/manifest.webmanifest` are never used. | Compare `README.md` "What works" / "Current intentional MVP limits" against `supabase/migrations/20260730220000_add_playtest_turn_handoff.sql:205`, `20260802173100_complete_stabilization_functions.sql:222-240`, and `grep -rn PwaRegister app components`. **Backlogged (P1-task9)** with finding 13, so docs and code change together. Note that findings 1, 10 and 11 have already made (c) and (d) stale in the other direction: scoring is group-local everywhere now, and the answer is no longer leaked through option order — though `game_submit_answer` does still return `correct_answer` on every response, including the mid-streak one — the README now says so. | fixed (5bc9741) |
| 19 | papercut | `game_begin_action` + test-mode turns | An off-turn player in a test league is rejected with **"No moves remaining"**, not "It is not your turn". `refresh_player_actions` zeroes the off-turn player's actions and `game_begin_action` checks the balance before the `enforce_test_turn_session` trigger ever fires, so the turn-specific message is unreachable for claim/attack/fortify. **Fixed:** `game_begin_action` checks the turn before the move balance now, so the turn-specific message is the one that surfaces. Defend stays exempt, as the trigger already had it. | `tests/db/audit.test.ts` "test-mode turn rotation blocks off-turn moves, rotates on end_test_turn, and allows off-turn defense" — the probe now requires "It is not your turn". | fixed (2a31dc2) |
| 20 | papercut | `pick_next_question` (20260802173000) | When a territory's pool is exhausted for the player, the fallback branch (`order by attempt_count asc, random()`) ignores both the 7-day exclusion and the session tier, so it can re-serve the question the player just answered — including inside the same attack streak, making the remaining answers free. Harmless with the full 11-question bank; reachable once finding 9's quarantines bite. **Fixed:** the fallback now excludes anything this session already served. A last-resort branch keeps the old behavior only when a territory has fewer active questions than the streak needs answers — repeating beats raising mid-streak, which would roll back the answer the player just got right. | `tests/db/audit.test.ts` "an exhausted question pool does not re-serve the same question inside one streak". Before the fix: deactivate every question for a territory except two, mark both as already seen, then answer correctly mid-streak — the same question came back and the streak completed. | fixed (4769e4f) |
| 21 | bug | `pick_next_question` (20260802173000) vs `territory-game-v2.tsx` | The adaptive selector sorts by a question's *observed* difficulty — once a row has 5+ attempts it is treated as tier 1 above a 75% correct rate and tier 3 below 45% — but two downstream consumers still read the row's *stored* `tier`. The answer timer is `case when v_question.tier = 3 then 45s else 30s end`, so an empirically hard question stored as tier 1 gets 30 seconds while an easy one stored as tier 3 gets 45; and the UI header prints "TIER {question.tier}", so a two-answer attack can be labelled "TIER 3" or "TIER 1". Selection and presentation disagree about what tier means. | Discovered as intermittent failure of `tests/db/audit.test.ts` probe 3 (~40% of suite runs, never in isolation): a tier-2 attack served a stored-tier-3 question once the bank accumulated history. Confirmed: `select tier, count(*) filter (where attempt_count >= 5 and correct_count::numeric/attempt_count >= .75) from questions where territory_id='OR' group by tier` → 2 of the 4 tier-3 rows had flipped to adaptive tier 1. The probe now asserts `required_correct` (the real cost) and no longer asserts the served row's tier. **Fixed in two parts.** `9d4a3c9` made `public.adaptive_tier(tier, attempts, correct)` the single definition and taught selection, the answer timer and `pick_next_question`'s payload to read it — but that covered the *serve* path only. `get_my_active_session` still returned `questions.tier`, and since `territory-game-v2.tsx` overwrites its live operation from that RPC on every realtime event and every 20s poll, the "TIER n" header flipped back to the stored tier seconds after serve. The follow-up stores the served tier on `question_attempts.served_tier` (beside the `expires_at` already handled that way) and returns that on resume, so serve and resume cannot disagree — recomputing on resume would not have been enough, since `attempt_count` moves as other players answer and the tier could change under the player mid-question. | fixed (9d4a3c9, 01b5f34) |
| 22 | blocker | `hooks/use-game-state.ts` `loadGroups` (pre-existing; identical logic was in the monolith) | A stale `localStorage.territory_group` wedges the whole app on the "Syncing the map" spinner with no recovery. `loadGroups` sets `groupId = preferred ?? saved ?? active ?? rows[0]` — trusting the saved id **without checking it is in the user's group list**. If that id names a group the current user is not a member of (removed from the group, group deleted, shared browser, or a different account on the same browser), `loadSnapshot` calls `group_snapshot` for it, the RPC raises `Group access denied`, and the error branch returns without ever calling `setSnapshot`, so `snapshot` stays null forever. The 20s recovery poll can't help — it only starts once a snapshot exists. `GameRuntimeControls` dodges this because its own loader does `groups.find(r => r.id === saved) ?? groups.find(active) ?? groups[0]`, falling back when the saved id is absent — which is why the turn banner rendered while the main view hung. Surfaced during the controller's Task 11 click-through: signing in as a fresh local user whose browser held a prior session's group id. | Live local stack: set `localStorage.territory_group` to any group id the signed-in user does not belong to, reload → permanent "Syncing the map"; `group_snapshot` returns 200 server-side but the coordinator's `groupId` (read from the React fiber) is the stale id, not the user's real group. Clearing the key and reloading renders the lobby correctly. | open (fix in progress) |
...
Updated after Task 8. `npm run test:db` is green at **36 pass, 0 skipped, 0
fail** across three files, and `npm test` at 19.

- `tests/db/audit.test.ts` — the audit probes. Every probe Task 7 left skipped is
  now un-skipped or replaced by one that asserts the fixed behavior; nothing in
  the suite is skipped any more.
- `tests/db/signup.test.ts` — new. Drives the `test-signup` edge function over
  HTTP against the local stack (findings 14 and 15). The local edge runtime
  caches the module, so `docker restart supabase_edge_runtime_HomeTerritory`
  after editing the function; `supabase db reset` does not restart it.
- `tests/cron-auth.test.ts` — new, and the only probe that needs no stack
  (finding 2 became a pure function in `lib/cron-auth.ts`).
- Findings 6 and 7 are guarded permanently rather than one-off: "no
  security-definer function in public is executable by anon" enumerates the
  schema through `public.security_definer_grants()` on every run.
- Finding 16 has no automated probe — `npm run lint` is the probe. It now runs
  instead of crashing, and reports 3 `react-hooks/set-state-in-effect` errors
  plus 1 `exhaustive-deps` warning in `components/territory-game-v2.tsx`,
  backlogged as P1-task9.
- Findings 12, 13 and 18 are backlogged, not fixed; see
  `docs/superpowers/backlog.md`.
- Finding 17 was reproduced in the browser by the controller; the recommendation
  above stands — no fix.

Three probes park a whole territory's question bank (findings 1, 9 and 21) and
