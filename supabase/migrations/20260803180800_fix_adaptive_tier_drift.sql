...
-- -- once a row has 5+ attempts it counts as tier 1 above a 75% correct rate and
-- tier 3 below 45% -- but two downstream consumers still read the row's *stored*
-- `tier`. The answer timer was `case when tier = 3 then 45s else 30s end`, so an
-- empirically hard question stored as tier 1 got 30 seconds while an easy one
-- stored as tier 3 got 45; and the UI header prints "TIER {question.tier}", so a
-- two-answer attack could be labelled "TIER 3". Selection and presentation
-- disagreed about what tier means.
--
-- One helper now computes the adaptive tier, and selection, the timer and the
-- payload all read it.

create or replace function public.adaptive_tier(p_tier integer, p_attempts integer, p_correct integer)
returns integer
language sql
immutable
set search_path = pg_catalog
as $$
  -- A question needs 5 answers before its observed rate is worth anything; above
  -- 75% correct it plays as tier 1, below 45% as tier 3, otherwise as authored.
  select case
    when p_attempts >= 5 and p_correct::numeric / nullif(p_attempts, 0) >= .75 then 1
    when p_attempts >= 5 and p_correct::numeric / nullif(p_attempts, 0) < .45 then 3
    else p_tier
  end;
$$;

revoke all on function public.adaptive_tier(integer, integer, integer) from public, anon, authenticated;
grant execute on function public.adaptive_tier(integer, integer, integer) to service_role;
